        ...
        if inv_filter + aug_random_filter < len(possible_x):
            ...
            if inv_filter:
                approx_x = self.inv_predict(
                    best + (np.abs(best) * np.random.normal(1.2, 0.05)),
...
class AskTellFewShotTopk(AskTellFewShot):
    ...
    def _setup_inverse_prompt(self, example: Dict):
        # Inverse prompt: y shows full phase context, x is procedure only
        prompt_template = PromptTemplate(
    ...
    def _tell(self, x, y: float, alt_ys: Optional[List[float]] = None) -> Dict:
        """Tell the optimizer about a new example.
        ...
        Forward prompt (regression): sees full phase context via format_x,
            predicts scalar MoC wf.
        Inverse prompt (design):     y shows full phase label with all sigmas,
            x is the procedure string only.
        """
        ...
        # ── Forward example: full phase context in x, scalar y ───────────
        example_dict = dict(
            x=self.format_x(x),   # x_formatter handles dict or str
            y=self.format_y(y),
            y_name=self._y_name,
        )
        self._ys.append(y)

        # ── Inverse example: full phase label in y, procedure only in x ──
        if isinstance(x, dict):
            # Build rich y label showing all phases + sigmas
            inv_y = (
                f"MoC weight fraction: {x['MoC_wf']:.1f}%, "
                f"sigma: {x['MoC_sigma']:.3f}%. "
            ...
            # Plain string fallback — original behaviour
            inv_y = self.format_y(y)
            inv_x = self.format_x(x)
    ...
    def _predict(self, queries: List[str], system_message: str) -> List[DiscreteDist]:
        if not system_message:
            warnings.warn(
                ...
                "Not clearly specifying the task for the LLM usually decreases "
